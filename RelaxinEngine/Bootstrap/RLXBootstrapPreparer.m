...
@implementation RLXBootstrapPreparer {
    ...
    BOOL _requiresExistingBootstrap;
...
- (instancetype)initWithKernelAccess:(nullable RLXKernelAccess *)kernelAccess
           ...
           requiresExistingBootstrap:(BOOL)requiresExistingBootstrap
    ...
    if (self) {
        ...
        _requiresExistingBootstrap = requiresExistingBootstrap;
...
- (nullable NSError *)prepare {
    ...
    if (_requiresExistingBootstrap && !installedRoot) {
        return rlx_bootstrap_preparation_error(
            @"restore_requires_existing_bootstrap",
            ENOENT,
            @"restore_mode=existing_required\ninstalled_root=false",
            nil);
    }
